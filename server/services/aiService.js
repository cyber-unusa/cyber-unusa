...
export const getAiChatResponseService = async (question) => {
  if (!question) throw new Error("Belum ada pertanyaan");
  ...
  if (activeKegiatan.length > 0) {
    ...
    activeKegiatan.forEach((k, index) => {
      kegiatanContext += `\n${index + 1}. Judul: ${k.title}. Deskripsi singkat: ${k.description}. Link Daftar: ${k.link}`;
    });
  }
